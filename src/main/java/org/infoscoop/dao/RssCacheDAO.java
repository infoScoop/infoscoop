...
public class RssCacheDAO  extends HibernateDaoSupport {
    ...
    public void insertCache(String uid, String url, int pageNumber, String data){
		try {
			insertCache( uid,url,pageNumber,data.getBytes("UTF-8") );
		} catch (UnsupportedEncodingException e) {
			log.error("", e);
		}
	}
    public void insertCache(String uid, String url, int pageNumber, byte[] data ) {
    ...
    public void deleteUserCache( String uid ) {
    	...
    	String queryString = "delete from Rsscache where Id.Uid = ?";
    ...
    public void deleteCacheByUrl( String uid, String url ) {
    	...
    	String queryString = "delete from Rsscache where Id.Uid = ? and Id.UrlKey = ?";
    	
    	super.getHibernateTemplate().bulkUpdate( queryString,
    			new Object[] { uid,url_key } );
    	
        if(log.isInfoEnabled())
        	log.info("deleteCacheByUid [ "+uid+"] successfully.");
    }
    
    public InputStream getCache(String uid, String url, int pageNumber){
    	if(log.isInfoEnabled()){
	...
	public List getCaches(String uid, String url) {
		if(log.isInfoEnabled()){
