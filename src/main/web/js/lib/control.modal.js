...
Object.extend(Control.Modal,{
	...
	load: function(){
		if(!Control.Modal.loaded){
			...
			Control.Modal.behindIframe = $(document.createElement('iframe'));
			Control.Modal.behindIframe.id = 'modal_container_biframe';
			...
			if(Browser.isIE){
				Event.observe(Control.Modal.container, "resize", Control.Modal.resizeIframe, false);
			}else{
				Control.Modal.behindIframe.style.width = 0;
				Control.Modal.behindIframe.style.height = 0;
				Control.Modal.behindIframe.style.left = 0;
				Control.Modal.behindIframe.style.top = 0;
			}
			Control.Modal.behindIframe.hide();
			document.getElementsByTagName('body')[0].appendChild(Control.Modal.behindIframe);
	...
	resizeIframe: function(){
		...
		Control.Modal.behindIframe.style.left = Control.Modal.container.offsetLeft + "px";
		Control.Modal.behindIframe.style.top = Control.Modal.container.offsetTop + "px";
		Control.Modal.behindIframe.style.width = Control.Modal.container.offsetWidth;
		Control.Modal.behindIframe.style.height = Control.Modal.container.offsetHeight;
	},
	center: function(){
...
Object.extend(Control.Modal.prototype,{
	...
	initialize: function(element,options){
		...
		this.position = function(){
			...
			if(!Browser.isIE){
				Control.Modal.behindIframe.setStyle({
					height: Control.Modal.getDocumentHeight() + 'px',
					width: Control.Modal.getDocumentWidth() + 'px'
				});
			}
	...
	update: function(html){
		// 16:22 2007/04/18 modified by nishiumi
//		Control.Modal.container.update(html);
		if(typeof html == "string"){
			Control.Modal.container.update(html);
		}else{
			if(Control.Modal.container.firstChild){
				Control.Modal.container.replaceChild(html, Control.Modal.container.firstChild);
			}else{
