  ...
  return /** @scope gadgets.io */ {
    ...
    makeRequest: function(url, callback, opt_params) {
      ...
      var paramData = {
		...
		'IS_AUTH_BASIC_USERNAME' : params.IS_AUTH_BASIC_USERNAME || "",
		'IS_AUTH_BASIC_PASSWORD' : params.IS_AUTH_BASIC_PASSWORD || ""
        };
      ...
      if (auth === 'oauth' || auth === 'signed' || auth === 'oauth2') {
        ...
        for (var opt in params) {
          if (params.hasOwnProperty(opt)) {
            if (opt.indexOf('OAUTH_') === 0 || opt === 'code' || opt.indexOf("OAUTH2_") === 0) {
